...
"#;
    ...
    assert!(mixin_code.contains(
        "nullableList: freezed == nullableList ? this.nullableList : nullableList as List<String>?"
    ));
    assert!(mixin_code.contains(
        "nullableMap: freezed == nullableMap ? this.nullableMap : nullableMap as Map<int, String>?"
    ));
}
...
"#;
    ...
    assert!(
        mixin_code.contains("_ConstTest ({required this.i, this.data = 'hello'}) : super._();")
    );
    assert!(mixin_code.contains("@override\n  final int i;"));
    assert!(mixin_code.contains("@override\n  final String data;"));
}
...
"#;
    ...
    assert!(
        mixin_code.contains(
            "nullableInt: freezed == nullableInt ? this.nullableInt : nullableInt as int?"
        )
    );
    assert!(mixin_code.contains(
        "nullableList: freezed == nullableList ? this.nullableList : nullableList as List<String>?"
...
"#;
    ...
    assert!(
        mixin_code
            .contains("GenericsTest copyWith({List<int>? ints, Map<String, List<int>>? complex});")
    );
    assert!(mixin_code.contains("ints: ints == null ? this.ints : ints as List<int>"));
    assert!(
        mixin_code.contains(
            "complex: complex == null ? this.complex : complex as Map<String, List<int>>"
        )
    );
}
...
abstract class JsonConverterTest with _$JsonConverterTest {
  ...
  JsonConverterTest._();
  factory JsonConverterTest.fromJson(Map<String, dynamic> json) =>
      _$JsonConverterTestFromJson(json);
}
"#;
    ...
    assert!(mixin_code.contains(
        "duration: const DurationJsonConverter().fromJson((json['duration'] as num).toInt())"
    ));
    ...
    assert!(
        mixin_code.contains(
            "numbers: const ListJsonConverter().fromJson((json['numbers'] as num).toInt())"
        )
    );
    ...
    assert!(
        mixin_code.contains("'duration': const DurationJsonConverter().toJson(instance.duration)")
    );
    assert!(
        mixin_code.contains(
            "'nullableField': const CustomJsonConverter().toJson(instance.nullableField)"
        )
    );
...
"#;
    ...
    assert!(mixin_code.contains(
        "  factory _BooleanTest.fromJson(Map<String, dynamic> json) => _$BooleanTestFromJson(json);"
    ));
    assert!(mixin_code.contains(
    ...
    assert!(
        mixin_code
            .contains("flags: (json['flags'] as List<dynamic>).map((e) => e as bool).toList()")
    );
    assert!(mixin_code.contains(
        "optionalFlags: (json['optionalFlags'] as List<dynamic>?)?.map((e) => e as bool).toSet()"
    ));
